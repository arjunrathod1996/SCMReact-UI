...
export const authAPI = {
  login: async (email: string, password: string): Promise<AuthResponse> => {
    ...
    const response = await axios.post('http://localhost:8080/login/authenticate', { email, password });
  ...
  sendOtp: async (phoneNumber: string): Promise<string> => {
    const response = await axios.post(`http://localhost:8080/auth/generate-otp/${phoneNumber}`);
    return response.data;
  },

  loginWithPhone: async (phoneNumber: string, otp: string): Promise<AuthResponse> => {
    const response = await axios.post(`http://localhost:8080/auth/verify-otp/${phoneNumber}/${otp}`);
  ...
  register: async (userData: RegisterRequest, token: string): Promise<any> => {
    const response = await axios.post('http://localhost:8080/admin/register', userData, {
...
export const userAPI = {
  getUserById: async (userId: string, token: string): Promise<GetUserResponse> => {
    const response = await axios.get(`http://localhost:8080/admin/get-users/${userId}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    return response.data;
  },
  ...
  updateUser: async (userId: string, userData: UserProfile, token: string): Promise<any> => {
    const response = await axios.put(`http://localhost:8080/admin/update/${userId}`, userData, {
      headers: { Authorization: `Bearer ${token}` }
    });
    return response.data;
  },
